...
class ChatController(BaseController[ChatDTO]):
    ...
    def formart_to_execure_dices(
        self, identity: str, dices: list[DiceShowDTO], chat_id: str | None
    ):
        chat_dto = self.get_by_id(id=chat_id) if chat_id else ChatDTO(user_id="lucas")
        one = 0
        two = 0
        m = "voce acabou de rolar os dados e tirou "
        for dice in dices:
            if dice.show == "1":
                one += 1
            if dice.show == "2":
        ...
        message = f"Seu personagem sendo o {identity}," + message_dices
